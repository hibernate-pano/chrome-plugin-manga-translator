...
export class DeepSeekProvider extends AIProvider {
  ...
  async translateText(request) {
    ...
    if (typeof request === 'string' || Array.isArray(request)) {
      ...
      options = arguments[2] || {};
      console.warn('[DeepSeekProvider] 使用了旧的 translateText 调用方式，建议使用新的请求对象格式');
    } else {
      // 新的统一接口
      text = request.text;
    ...
    try {
      ...
      console.log('[DeepSeekProvider] 开始翻译:', {
      ...
      for (let i = 0; i < textsToTranslate.length; i += batchSize) {
        ...
        const result = await APIErrorHandler.withRetry(async () => {
          console.log('[DeepSeekProvider] 发送 API 请求:', {
            url: apiUrl,
            model: this.config.chatModel,
          ...
          console.log('[DeepSeekProvider] API 响应状态:', response.status);
          ...
          console.log('[DeepSeekProvider] 翻译成功:', {
      ...
      console.error('[DeepSeekProvider] 翻译失败:', error);
      throw error;
    }
  }
